...
import com.learntoflutter.flutter_embed_unity_android.platformView.MissingUnityViewFactory
...
class FlutterEmbedUnityAndroidPlugin : FlutterPlugin, ActivityAware {
    private val unityLibraryAvailable =
        runCatching { Class.forName("com.unity3d.player.UnityPlayer") }.isSuccess
    ...
    private val methodCallHandler by lazy { SendToUnity() }
    // This lifecycle observer works around an issue with Unity freezing sometimes when
    // the app is brought back into foreground
    private val resumeUnityOnActivityResume by lazy { ResumeUnityOnActivityResume() }
        ...
        flutterPluginBinding
            ...
            .registerViewFactory(
                ...
                if (unityLibraryAvailable) {
                    ...
                    Log.w(logTag, "Unity library not found. Registering fallback view.")
                    MissingUnityViewFactory()
                }
            )
    }
    ...
    override fun onAttachedToActivity(binding: ActivityPluginBinding) {
        ...
        if (!unityLibraryAvailable) {
            Log.w(logTag, "Unity library not found. Skipping Unity activity setup.")
            return
        }
